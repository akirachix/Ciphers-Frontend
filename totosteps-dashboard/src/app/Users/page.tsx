...
//       <h1 className="text-[50px] font-bold font-nunito text-black text-center">Totosteps Users</h1>
//       <br/> 
...
//           <div className="text-pink-500 font-bold text-[30px]">ID</div>
...
const useUsers = () => {
  ...
  useEffect(() => {
    const initializeUsers = async () => {
      const savedUsers = localStorage.getItem('users');
      if (savedUsers) {
        setUsers(JSON.parse(savedUsers).reverse());
      } else {
        const apiUsers = await fetchUsersFromAPI();
        const filteredUsers = apiUsers.filter((user: User) => user.first_name && user.last_name);
        if (filteredUsers.length > 0) {
          ...
          localStorage.setItem('users', JSON.stringify(reversedUsers));
        }
      }
      setLoading(false);
    };
  ...
  const saveUsersToLocalStorage = (updatedUsers: User[]) => {
    const reversedUsers = [...updatedUsers].reverse();
    localStorage.setItem('users', JSON.stringify(reversedUsers));
    setUsers(reversedUsers);
  };
...
const UserList: React.FC = () => {
  ...
  const getInitialsBackgroundColor = (initials: string) => {
    const hue = Math.floor((initials.charCodeAt(0) + initials.charCodeAt(1)) % 360);
